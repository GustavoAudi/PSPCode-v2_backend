...
class PhaseInstance < ApplicationRecord
  ...
  def defect_deletion_consistence
    return unless project_delivery.phase_instances.where(phase_id: phase_id_was).count == 0
  ...
  def update_elapsed_time
    ...
    if start_time.present? && end_time.present?
      self.elapsed_time = ((end_time.to_i - start_time.to_i) / 60) - interruption_time.to_i
    else
      self.elapsed_time = 0
    end
  end
